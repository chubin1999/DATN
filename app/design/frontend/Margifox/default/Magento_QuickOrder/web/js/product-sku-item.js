    ...
    $.widget('mage.productSkuItem', {
        ...
        _addByAjax: function () {
            ...
            if(item.qty == '') {
                ...
                $('.box.box-items').find('.fields.additional.deletable-item[data-role="new-block"]').each(function( index,element ) {
                    var sku_ = $(this).find('.field.sku.required.ui-front').find('.control').find('input').val();
                    var qty_ = $(this).find('.field.qty.required').find('.control.qty').find('input').val();
                    if(item.sku == sku_ && qty_ != '') {
                        qty_ = parseInt(qty_);
                        total_qty = total_qty + qty_;
                        $(this).remove();
                    }
                });
                if(total_qty > 0) {
                    item.qty = total_qty+1;
                }
            }
