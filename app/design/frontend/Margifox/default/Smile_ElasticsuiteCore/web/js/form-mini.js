    ...
    $.widget('smileEs.quickSearch', $.mage.quickSearch, {
        ...
        _create: function () {
            ...
            $(document).click(function (e) {
                var searchAutocompletePopup = $("#search_autocomplete"),
                    inputSearch = $('#search_mini_form #search');
                if($.trim(searchAutocompletePopup.html()) != ''){
                    if ((!searchAutocompletePopup.is(e.target) && searchAutocompletePopup.has(e.target).length === 0) &&
                    (!inputSearch.is(e.target) && inputSearch.has(e.target).length === 0)
                    ) {
                        searchAutocompletePopup.hide();
                        self._updateAriaHasPopup(false);
                        $('#search_mini_form .input-text').val('');
                        $('.smile-elasticsuite-autocomplete-result').html('');
                        searchAutocompletePopup.parents('.field.search').removeClass('show');
                        $('#search_mini_form .actions .action.search').show();
                        $('#search_mini_form .actions-delete').hide();
            ...
            {
                e.preventDefault();
                var q =  $('#search_mini_form .input-text').val(),
                url = mageUrl.build('catalogsearch/result') + '?q='+ q;
                window.location.href = url;
            });
            $(document).on('click','#search_mini_form .actions .action.search',function(e)
            {
                e.preventDefault();
                var q =  $('#search_mini_form .input-text').val(),
                url = window.location.origin + '/catalogsearch/result/?q='+ q;
                window.location.href = url;
            });
        ...
        _getSectionTitle: function(type, data) {
            ...
            if (this.titleRenderers && this.titleRenderers[type]) {
                if (type == 'cms_page') {
                    title = $('<dt role="listbox" class="autocomplete-list-title title-' + type + '">Pages</dt>');
                } else {                    
        ...
        _onPropertyChange: _.debounce(function () {
            ...
            if ($('#search_mini_form #search').val() != '') {
                $('#search_mini_form .actions-delete').show();
            ...
            if (value.trim().length >= parseInt(this.options.minSearchLength, 10)) {
                ...
                this.currentRequest = $.ajax({
                    ...
                    success: $.proxy(function (data) {
                        ...
                        self.searchForm.addClass('showing-dropdown');
                        content.append('<div class="search-col-left"></div>');
                        content.append('<div class="search-col-right"></div>');
                        var isEmptyLeft = true;
                        var isEmptyRight = true;

                        var add_category = true;
                        $.each(data, function(index, element) {
                            ...
                            if (!lastElement || (lastElement && lastElement.type !== element.type)) {
                                if (element.type == 'product') {
                                    isEmptyRight = false;
                                    content.children('.search-col-right').append(sectionDropdown);
                                    if (data[0].num_results > 0) {
                                        var numCount = '<div class="search-col-right-bottom">' + '<a href="#" class="search-view-all">' + 'View all ' + data[0].num_results + '</a>' + '</div>'
                                    }
                                    else {
                                        var numCount = '<div class="search-col-right-bottom">' + '<a href="#" class="search-view-all">' + 'View all ' + data.length + '</a>' + '</div>'
                                    }
                                    content.children('.search-col-right').append(numCount);
                                } else {
                                    isEmptyLeft = false;
                                    content.children('.search-col-left').append(sectionDropdown);
                                }
                            }
                        ...
                        if (isEmptyRight) {
                            content.children('.search-col-right').addClass('empty').append('<h3>Categories</h3><p class="no-result">No results</p><h3>Pages</h3><p>No results</p></p>');
                            $('.search-autocomplete').addClass('no-products-found');
                        } else {
                            // custom add category none
                            if(add_category) {

                                var html_add_category_none =
                                '<dl role="listbox" class="autocomplete-list category">' +
                                    '<dt role="listbox" class="autocomplete-list-title title-category">Categories</dt>' +
                                    '<dd class="selected">' +
                                        '<span class="qs-option-name">No results</span>' +
                                    '</dd>' +
                                '</dl>';

                                var search_col_left = content.children('.search-col-left');

                                $(html_add_category_none).appendTo(search_col_left);

                            }
                            // end custom add category none
                            $('.search-autocomplete').removeClass('no-products-found');
                        }
                        if (isEmptyLeft) {
                            content.children('.search-col-left').addClass('empty').append('<div class="search-col-right-bottom">No products for query "'+value+'"</div>');
                        }
                        ...
                        if ($(window).width() < 768) {
                            $('body').addClass('search-active-mobile');
                            $('html').addClass('search-active-mobile');
                        }
                    ...
                    complete : $.proxy(function () {
                        var viewall = window.location.origin+'/catalogsearch/result/?q='+ value;
                        var html = '<dd class="seeall"><a href="'+viewall+'">' + "See all results"+'</a></dd>';
                        var lastResult = $('#search_autocomplete .autocomplete-list');
                        lastResult.append(html);
                        this.searchForm.removeClass('processing');
                    }, this)
                });
            } else {
                this.searchForm.removeClass('showing-dropdown');
                this._resetResponseList(true);
                this.autoComplete.hide();
                this._updateAriaHasPopup(false);
                this.element.removeAttr('aria-activedescendant');
                $('body').removeClass('search-active-mobile');
                $('html').removeClass('search-active-mobile');
            }
        }, 250),
