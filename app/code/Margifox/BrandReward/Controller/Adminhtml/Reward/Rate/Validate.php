...
namespace Margifox\BrandReward\Controller\Adminhtml\Reward\Rate;

class Validate extends \Magento\Reward\Controller\Adminhtml\Reward\Rate\Validate
{
    /**
     * Validate Action
     *
    ...
    public function execute()
    {
        ...
        if (!isset(
            ...
            /** @var \Margifox\BrandReward\Model\Reward\Rate $rate */
            $rate = $this->_initRate();
            $isRateUnique = $rate->getIsRateUniqueForBrandToCurrent(
                $post['website_id'],
                $post['customer_group_id'],
                $post['direction'],
                $post['brand_id']
